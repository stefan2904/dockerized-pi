...
./pi.sh --env GITLAB_TOKEN=glpat-... # pass an extra environment variable
./pi.sh --env-file .pi.env           # pass an extra Docker env-file from $PWD
...
| =--env KEY=VALUE=, =--env=KEY=VALUE=, =--docker-env KEY=VALUE=, =--docker-env=KEY=VALUE= | Pass an additional environment variable to =docker run=. Use =KEY= without a value to copy from the host environment. |
| =--env-file PATH=, =--env-file=PATH=, =--docker-env-file PATH=, =--docker-env-file=PATH= | Pass an additional Docker env-file. Relative paths are resolved from the directory where =pi.sh= was invoked. Additional env-files are passed after this repo's =.env=. |
...
Copy =.env.template= to =.env= and set only what you need. The file is passed to Docker with =--env-file= and is ignored by git. You can also pass per-run values with launcher options such as =--env FOO=bar= or =--env-file ./local.env=.
